...
begin
  ...
  if base_username is null then
    base_username := nullif(trim(split_part(coalesce(new.email, ''), '@', 1)), '');
  end if;
  ...
  if candidate_username is not null then
    while exists (select 1 from public.profiles where username = candidate_username and id <> new.id) loop
      suffix := suffix + 1;
      candidate_username := left(base_username, greatest(1, 50 - length(suffix::text) - 1)) || '_' || suffix::text;
    end loop;
  end if;
...
-- After registering the real administrator through Supabase Auth, promote that
-- specific profile with an explicit, administrator-selected email address:
