...
-- Security-definer helpers prevent recursive RLS evaluation on profiles.
...
-- The configured administrator can safely bootstrap its profile after Auth login.
...
begin
  ...
  if current_email <> lower('punichinmayms12@gmail.com') then return false; end if;
  ...
  values (
    ...
    current_email, 'chinmayms12', 'admin'
  )
  on conflict (id) do update set email = excluded.email, username = 'chinmayms12', role = 'admin';
  return true;
end;
$$;
