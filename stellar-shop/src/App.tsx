...
import { replaceProducts } from '@/catalog';
...
function CatalogBootstrap({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      else replaceProducts(result.data);
      setReady(true);
    };
    void load();
  ...
  return <>{error && <div className="mx-auto max-w-7xl px-4 pt-3 lg:px-6"><div className="rounded-xl border border-warning-300 bg-warning-500/10 p-3 text-sm text-warning-700 dark:text-warning-300">Catalog could not be loaded from Supabase: {error}</div></div>}{children}</>;
}
