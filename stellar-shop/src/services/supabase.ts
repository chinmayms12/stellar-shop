...
export async function resolveAdminIdentifier(identifier: string) {
  ...
  // IMPORTANT: do not call a database RPC here. Username resolution happens
  // before authentication, so an RPC that is missing from an older Supabase
  // project can never be allowed to block the login form. The actual password
  // check is still performed by Supabase Auth below, and admin authorization
  // is checked against the authenticated user's profile after sign-in.
  const knownAdminUsername = fallbackUsername || 'chinmayms12';
  const knownAdminEmail = fallbackEmail || 'punichinmayms12@gmail.com';
