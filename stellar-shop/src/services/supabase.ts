...
export async function resolveAdminIdentifier(identifier: string) {
  ...
  const fallbackUsername = String(import.meta.env.VITE_ADMIN_USERNAME || '').trim().toLowerCase();
  const fallbackEmail = String(import.meta.env.VITE_ADMIN_EMAIL || '').trim().toLowerCase();
  if (fallbackUsername && fallbackEmail && value === fallbackUsername) {
    return { email: fallbackEmail, error: null };
  }

  // Username lookup is normally handled by the secure SQL RPC. Keep a small,
  // deployment-configured fallback so an older Supabase database cannot turn
  // the login form into a useless [object Object] error when that RPC has not
  // yet been installed. The password is still verified by Supabase Auth and
  // admin authorization is checked again after sign-in.
  try {
    const { data, error } = await supabase.rpc('lookup_admin_email_by_username', { p_username: value });
    if (!error) {
      const email = typeof data === 'string' ? data.trim().toLowerCase() : '';
      if (email) return { email, error: null };
    }

    if (error) return { email: null, error: normalizeAuthError(error) };
    return { email: null, error: new Error('No administrator account was found for that User ID.') };
  } catch (error) {
    return { email: null, error: normalizeAuthError(error) };
  }
}
...
function normalizeAuthError(error: unknown): Error {
  ...
  if (lower.includes('unexpected end of json') || lower.includes("failed to execute 'json'")) {
    return new Error('Supabase returned an empty/invalid response. The Vercel Supabase connection is being repaired; please try again.');
  }
