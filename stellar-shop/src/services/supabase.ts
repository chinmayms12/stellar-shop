...
export async function bootstrapConfiguredAdmin() {
  ...
  try {
    ...
    return { ok: data === true, error: data === true ? null : new Error('The configured administrator account could not be promoted. Run supabase/fix_admin_login.sql once in Supabase SQL Editor.') };
  } catch (error) {
...
export async function isCurrentUserAdmin() {
  ...
  } catch { /* fall back to the profile row below */ }
