...
function normalizeAuthError(error: unknown): Error {
  ...
  if (lower.includes('unexpected end of json') || lower.includes("failed to execute 'json'")) {
    return new Error('Supabase returned an invalid response. Verify the Supabase project URL/key and try again.');
  }
  if (lower.includes('failed to fetch') || lower.includes('networkerror') || lower.includes('load failed')) {
    return new Error('Cannot reach Supabase. Check that the Supabase project is active and the Vercel environment variables are correct.');
  }
...
export async function fetchPublicProducts() {
  if (!supabaseEnabled || !supabase) return { data: [] as Product[], error: new Error('Supabase is not configured.') };
  try {
    const { data, error } = await supabase.from('products').select('*').order('created_at', { ascending: false });
    if (error) return { data: [] as Product[], error };
    const mapped = (data ?? []).map((p: any) => ({
      id: String(p.id), name: String(p.name || ''), slug: String(p.slug || ''), brand: String(p.brand || ''), brandId: String(p.brand_id || p.brand || ''), category: String(p.category || ''), categoryId: String(p.category_id || p.category || ''),
      price: Number(p.price || 0), mrp: Number(p.mrp || 0), rating: Number(p.rating || 0), reviewCount: Number(p.review_count || 0), stock: Number(p.stock || 0),
      images: Array.isArray(p.images) ? p.images : [], colors: Array.isArray(p.colors) ? p.colors : [], highlights: Array.isArray(p.highlights) ? p.highlights : [], specs: Array.isArray(p.specs) ? p.specs : [], description: String(p.description || ''),
      warranty: String(p.warranty || ''), returnPolicy: String(p.return_policy || ''), delivery: String(p.delivery || ''), tags: Array.isArray(p.tags) ? p.tags : [], badge: p.badge || undefined,
      createdAt: p.created_at || new Date().toISOString(), reviews: Array.isArray(p.reviews) ? p.reviews : [], frequentlyBoughtTogether: Array.isArray(p.frequently_bought_together) ? p.frequently_bought_together : undefined,
    })) as Product[];
    return { data: mapped, error: null };
  } catch (error) {
    return { data: [] as Product[], error: normalizeAuthError(error) };
  }
}
