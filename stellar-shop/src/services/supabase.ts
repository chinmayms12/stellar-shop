...
export async function bootstrapConfiguredAdmin() {
  ...
  // Do not depend on a custom SQL RPC for the first administrator login.
  // Existing Stellar Shop databases already create a profile for every Auth
  // user, and the authenticated user is allowed to update their own profile.
  // This makes the login work even when the optional migration has never been run.
  try {
    const { data, error } = await supabase
      .from('profiles')
      .update({
        email: user.email,
        username: String(import.meta.env.VITE_ADMIN_USERNAME || CONFIGURED_ADMIN_USERNAME).trim(),
        role: 'admin',
      })
      .eq('id', user.id)
      .select('id,role')
    ...
    if (!data || data.role !== 'admin') {
      return { ok: false, error: new Error('The administrator profile does not exist yet. Please create the admin account through the normal signup flow once, then log in again.') };
    }
...
export async function isCurrentUserAdmin() {
  ...
  // Read the profile directly. This deliberately does not call a custom RPC,
  // because older Supabase projects may not have the optional SQL functions.
  try {
    const { data, error } = await supabase
      .from('profiles')
      .select('role')
