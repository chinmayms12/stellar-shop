...
function authError(message: string) {
  ...
  if (lower.includes('failed to fetch') || lower.includes('fetch failed') || lower.includes('networkerror') || lower.includes('network error') || lower.includes('load failed')) {
    return new Error('AUTH_NETWORK_ERROR');
  }
  if (lower.includes('captcha')) return new Error('CAPTCHA_ERROR');
...
export function getAuthErrorMessage(error: unknown) {
  ...
  switch (raw) {
    case 'AUTH_NETWORK_ERROR':
      return 'Cannot reach Supabase. Check your Vercel VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY, then redeploy. Also make sure the Supabase project is active.';
    case 'CAPTCHA_ERROR':
      return 'Security verification failed. Disable Turnstile in this deployment or configure the same Turnstile site key in Supabase.';
    case 'EMAIL_CONFIRMATION_REQUIRED':
      return 'Email confirmation is enabled in Supabase. Turn off Confirm email in Authentication → Providers → Email, or confirm the email before signing in.';
    case 'ACCOUNT_EXISTS':
...
export async function signInWithSupabase(email: string, password: string, captchaToken?: string) {
  ...
  try {
    ...
    if (result.error) return { ...result, error: authError(result.error.message), skipped: false };
    return { ...result, skipped: false };
  } catch (error) {
    return { data: { user: null, session: null }, error: authError(error instanceof Error ? error.message : String(error)), skipped: false };
  }
}
...
export async function signInWithOAuthProvider(provider: 'google' | 'linkedin_oidc' | 'github' | 'azure') {
  ...
  try {
    ...
    const redirectTo = configured && !/localhost|127\.0\.0\.1/i.test(configured) ? configured : window.location.origin;
    const result = await supabase.auth.signInWithOAuth({ provider, options: { redirectTo } });
    if (result.error) return { ...result, error: authError(result.error.message), skipped: false };
    return { ...result, skipped: false };
  } catch (error) {
    return { data: null, error: authError(error instanceof Error ? error.message : String(error)), skipped: false };
  }
}
...
export async function signUpWithSupabase(email: string, password: string, metadata: { name: string; mobile?: string; username?: string }, captchaToken?: string) {
  ...
  try {
    ...
    // Supabase can intentionally return a user with no identities for an email
    // that already exists when email enumeration protection is enabled.
    const duplicate = !!result.data.user && Array.isArray(result.data.user.identities) && result.data.user.identities.length === 0;
    if (duplicate) return { ...result, error: new Error('ACCOUNT_EXISTS'), skipped: false };

    // With Confirm email OFF, signUp should already return a session. If a
    // provider/configuration still returns no session, one controlled login
    // attempt makes the behavior deterministic without leaving an exception.
    if (result.data.user && !result.data.session) {
      const login = await signInWithSupabase(normalizedEmail, password, captchaToken);
      if (!login.error && login.data?.session) return { data: login.data, error: null, skipped: false };
      const loginMessage = login.error?.message || '';
      if (loginMessage === 'AUTH_NETWORK_ERROR') return { ...result, error: login.error, skipped: false };
      if (/confirm|verification|verified/i.test(loginMessage)) return { ...result, error: new Error('EMAIL_CONFIRMATION_REQUIRED'), skipped: false };
      // A no-session signup is valid when email confirmation is enabled.
      return { ...result, skipped: false };
    }
...
export async function touchUserActivity(userId: string, isLogin = false) {
  ...
  } catch { /* activity tracking must never break authentication */ }
}
