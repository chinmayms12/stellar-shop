...
import { offers, coupons } from '@/catalog';
import { productsByTag } from '@/catalog';
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 lg:px-6">
      ...
      <div className="overflow-hidden rounded-card gradient-stellar p-8 text-center text-white">
        ...
        <p className="mx-auto mt-2 max-w-md text-white/80">Current promotions configured by the store administrator.</p>
      </div>

      {/* Offer cards with countdown */}
      {offers.length > 0 ? <div className="mt-6 grid gap-4 md:grid-cols-2">
        {offers.map((o) => {
      ...
      </div> : <div className="mt-6 rounded-card border border-base bg-elevated p-10 text-center text-sm text-muted">No active offers have been configured yet.</div>}

      {/* Coupons */}
      {coupons.length > 0 && <section className="mt-10">
