...
import { resolveAdminIdentifier, signInWithSupabase, isCurrentUserAdmin, signOutSupabase } from '@/services/supabase';
...
export function AdminLogin() {
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);
    if(!supabaseEnabled){setBusy(false);dispatch(addToast({message:'Supabase is not configured for this deployment.',type:'error'}));return;}
    const resolved=await resolveAdminIdentifier(userId);
    if(resolved.error || !resolved.email){setBusy(false);dispatch(addToast({message:resolved.error?.message || 'Administrator account not found.',type:'error'}));return;}
    const result=await signInWithSupabase(resolved.email,password);
    if(result.error){setBusy(false);dispatch(addToast({message:result.error.message || 'Invalid administrator password.',type:'error'}));return;}
    const admin=await isCurrentUserAdmin();
    if(admin){setAdminSessionHint(true);setBusy(false);dispatch(addToast({message:'Admin login successful.',type:'success'}));navigate('/admin',{replace:true});return;}
    await signOutSupabase();setBusy(false);dispatch(addToast({message:'This account is not an administrator. Promote it in Supabase with role = admin.',type:'error'}));
  };
