...
export function AdminLogin() {
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);
    try {
      ...
      // The Auth password is already verified. For the one configured administrator,
      // repair a missing/customer profile safely through a security-definer RPC.
      const bootstrap = await bootstrapConfiguredAdmin();
      if (!bootstrap.ok && bootstrap.error) {
        const text = bootstrap.error.message.toLowerCase();
        const missingRpc = text.includes('bootstrap_admin_account') && (text.includes('does not exist') || text.includes('schema cache'));
        if (missingRpc) {
          await signOutSupabase();
          dispatch(addToast({message:'Supabase admin setup is incomplete. Run supabase/fix_admin_login.sql once in Supabase SQL Editor, then log in again.',type:'error'}));
          return;
        }
      }
      ...
      await signOutSupabase();
      dispatch(addToast({message:'Authentication succeeded, but this account is not authorized as an administrator. Run supabase/fix_admin_login.sql once in Supabase SQL Editor.',type:'error'}));
    } catch (error) {
