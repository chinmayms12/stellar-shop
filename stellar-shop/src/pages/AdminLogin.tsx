...
export function AdminLogin() {
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);
    try {
      ...
      // repair the existing profile without requiring a custom SQL RPC.
      const bootstrap = await bootstrapConfiguredAdmin();
      if (!bootstrap.ok) {
        await signOutSupabase();
        dispatch(addToast({message:bootstrap.error?.message || 'Administrator profile could not be prepared. Please check the Supabase profile for this account.',type:'error'}));
        return;
      }
