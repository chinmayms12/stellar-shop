...
export function AdminLogin() {
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);
    if(supabaseEnabled){const result=await signInWithSupabase(userId.trim(),password);if(!result.error){const admin=await isCurrentUserAdmin();if(admin){setAdminSessionHint(true);setBusy(false);dispatch(addToast({message:'Admin login successful.',type:'success'}));navigate('/admin',{replace:true});return;}await signOutSupabase();}}
