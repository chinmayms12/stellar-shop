...
import { getAuthErrorMessage, signInWithOAuthProvider, signInWithSupabase } from '@/services/supabase';
...
export function Login() {
  ...
  const socialLogin = async (provider: SocialProvider) => {
    ...
    setBusy(false);
    if (result.error) {
      dispatch(addToast({ message: getAuthErrorMessage(result.error), type: 'error' }));
    }
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (result.error || !result.data?.session) {
      dispatch(addToast({ message: getAuthErrorMessage(result.error) || 'Invalid email or password.', type: 'error' }));
      return;
    }
