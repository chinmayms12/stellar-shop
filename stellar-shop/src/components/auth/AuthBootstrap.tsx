  ...
  useEffect(() => {
    ...
    const sync = async (session: any) => {
      ...
      try {
        ...
        // A profile-table/network failure must not crash the React app.
        if (mounted) dispatch(loginSuccess({
          user: {
            id: u.id,
            name: u.user_metadata?.name || u.email?.split('@')[0] || 'Stellar User',
            email: u.email || '',
    ...
    void supabase.auth.getSession()
      ...
      .catch(() => { if (mounted) dispatch(logout()); });
