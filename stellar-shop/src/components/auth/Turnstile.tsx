...
export function Turnstile({ onVerify }: { onVerify: (token: string) => void }) {
  ...
  const enabled = String(import.meta.env.VITE_ENABLE_TURNSTILE || '').toLowerCase() === 'true';
  const siteKey = enabled ? (import.meta.env.VITE_TURNSTILE_SITE_KEY as string | undefined) : undefined;
  useEffect(() => {
    if (!siteKey) return;
    const render = () => {
